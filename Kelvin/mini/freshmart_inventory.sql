...
-- drop old tables first so we dont get errors
...
-- 3. find products expiring in next week with stock > 50
...
-- 4. dead stock - stuff that hasnt sold in 60 days

PROMPT ===== Dead Stock (No sales in last 60 days) =====

-- tried a subquery first but left join works better here
-- select ProductID, ProductName from Products where ProductID not in (select ProductID from SalesTransactions where SaleDate >= SYSDATE - 60);
...
-- 5. revenue by category for last month
